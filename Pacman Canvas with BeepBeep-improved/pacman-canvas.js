...
/* ----- Global Variables ---------------------------------------- */
        ...
        var monitors_array = new Array();
	var lsp = new LTLStringParser();
	//var monitor = lsp.parseFromString("G (document.getElementById('myCanvas').width == '540')");
	monitors_array.push(lsp.parseFromString("G (pacman.posY <= '200')"));
	monitors_array.push(lsp.parseFromString("F (pacman.posX >= '200')"));
	monitors_array.push(lsp.parseFromString("G (∀ x1 ∈ pacman.posX : (X (∀ x2 ∈ pacman.posX : (x2 >= x1))))"));
	...
	function Game() {
		...
		this.pauseResume = function () {
			if (!this.running) {
				...
				this.showMessage("Pause","Click to continue");
	...
	function Score() {
		...
		this.refresh = function(h) {
		  var verdict_string = "";
                  for (var i = 0; i < monitors_array.length; i++)
                  {
                    var mon = monitors_array[i];
		    //verdict = mon.getVerdict();
                    if (verdict == 0)
                      verdict_string += " <span class=\"beepbeep-verdict-false\"></span>";
                    else if (verdict == 1)
                      verdict_string += " <span class=\"beepbeep-verdict-inconclusive\"></span>";
                    else
                      verdict_string += " <span class=\"beepbeep-verdict-true\"></span>";
		  }
			//$(h).html("Score: "+this.score);
			$(h).html("Score: " + this.score + verdict_string);
		}
